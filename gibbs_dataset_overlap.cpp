...
int main(int argc, char *argv[])
{
    ...
    int num_lbl = atoi(argv[2]);
    int em_iter = atoi(argv[3]);
    int gibbs_iter_tot = atoi(argv[4]);
    double beta = atof(argv[5]);
    double lambda = atof(argv[6]);
    int step = atof(argv[7]);
    int HW = atof(argv[8]);
    int run_num = atof(argv[9]);
    int loop = atof(argv[10]);
    //char* input_string = argv[11];
    char* textfile = argv[11];
    FILE* fptr;
    if ((fptr = fopen(textfile, "r")) == NULL) {
    ...
    while (fgets(input_string, sizeof(input_string), fptr) != NULL ) {
        // if (strcmp(input_string, ) == 0 ) {
        //   break;
        // }
        // cout << strcmp(input_string,"") << endl;
        // fscanf(fptr, "%[^\n]", input_string);
        //char input_string[1024];
        char kILDfileName[1024];
        // fscanf(fptr, "%[^\n]", input_string);
        strcpy(kILDfileName, "clabels/");
        strcat(kILDfileName, input_string);
        cout << kILDfileName << endl;

        char* pch = strtok(input_string," _");
        pch = strtok (NULL, " _");
        int height = atoi(pch);
        ...
        kILDfileName[strlen(kILDfileName)-1] = '\0';

        cout << kILDfileName << endl;

        // read in ild file
        //char const *kILDfileName = "input/1188-133604-0029_117_-ild-noise.txt";
        ifstream ild_stream(kILDfileName);
        cout << ild_stream.fail() << endl;
        // store ild values in
    ...
    //    double vari[] = {30, 30}; // binary
        ...
        double mean_[] = {5, -5}; // binary
        double vari_[] = {1, 5}; // binary
    ...
    //    Dump3DVector(datacost, width, height, num_lbl, "output/datacost.txt");
        ...
        */
        ...
        char* pch = strtok(input_string," _");
        char fileName[1024];
        strcpy(fileName, "output_labels/");
        strcat(fileName, pch);
        strcat(fileName, "-new_labels_t.txt");
        // cout << fileName << endl;
        Dump2DVectorTranspose(global_labels, width, height, fileName); //"output/7729-102255-0024-new_labels_t.txt"
        }
